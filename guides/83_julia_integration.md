...
# Just use anofox_fcst_ts_forecast_by - no need for Julia threading
forecasts = DataFrame(DBInterface.execute(con, """
...
metrics = DataFrame(DBInterface.execute(con, """
    ...
    SELECT 
        ...
        anofox_fcst_ts_coverage(actuals, lower, upper) AS coverage
    FROM joined
"""))
...
function run_forecast_pipeline(input_file::String, output_file::String)
    ...
    # Forecast
    forecasts = DataFrame(DBInterface.execute(con, """
        SELECT * FROM anofox_fcst_ts_forecast_by('sales_prep', product_id, date, amount,
                                     'AutoETS', 28, {'seasonal_period': 7})
    """))
...
forecast = DataFrame(DBInterface.execute(con, """
    SELECT * FROM anofox_fcst_ts_forecast('sales', date, amount, 'AutoETS', 7,
