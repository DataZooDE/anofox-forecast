...
class TestPandasDateTypes(unittest.TestCase):
    ...
    def test_integer_date_column_ts_forecast(self):
        ...
        df = pd.DataFrame({'date_col': range(1, 31), 'value': [100.0 + i * 5 for i in range(1, 31)]})
    ...
    def test_datetime64_ts_forecast(self):
        ...
        df = pd.DataFrame(
            {'date_col': pd.date_range('2024-01-01', periods=30, freq='D'), 'value': [100.0 + i * 5 for i in range(30)]}
        )
    ...
    def test_ts_fill_gaps_datetime_dates(self):
        ...
        df = pd.DataFrame({'date_col': dates, 'series_id': ['series_1'] * 27, 'value': [100.0] * 27})
    ...
    def test_autoarima_integer_dates(self):
        ...
        df = pd.DataFrame({'date_col': range(1, 51), 'value': [100.0 + i * 5 + np.sin(i / 7) * 10 for i in range(50)]})
    ...
    def test_mixed_integer_types(self):
        ...
        # Regular int
        df1 = pd.DataFrame(
            {'date_col': pd.Series(range(1, 31), dtype='int32'), 'value': [100.0 + i * 5 for i in range(30)]}
        )
        ...
        # Bigint
        df2 = pd.DataFrame(
            {'date_col': pd.Series(range(1, 31), dtype='int64'), 'value': [100.0 + i * 5 for i in range(30)]}
        )
    ...
    def test_pandas_datetime64_without_conversion_fails(self):
        ...
        with self.assertRaises(Exception) as context:
            self.con.execute("SELECT COUNT(*) FROM ts_forecast(test_data, date_col, value, 'Naive', 10, NULL)")
