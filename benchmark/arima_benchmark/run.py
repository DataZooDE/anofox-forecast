...
from configs.arima import BENCHMARK_NAME, MODELS


def run_anofox(group: str = 'Daily'):
    """
    Run Anofox ARIMA benchmarks on M4 dataset.
...
def evaluate(group: str = 'Daily'):
    """
    Evaluate ARIMA model forecasts on M4 test data.
...
def run(group: str = 'Daily'):
    """
    Run complete ARIMA benchmark: anofox + evaluation.
    ...
    """
    print(f"{'='*80}")
    print(f"ARIMA BENCHMARK - M4 {group}")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"ARIMA BENCHMARK COMPLETE")
    print(f"{'='*80}")
