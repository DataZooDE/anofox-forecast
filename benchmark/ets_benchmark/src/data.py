"""
Data loading utilities for ARIMA benchmark.
...
def save_data(group: str, train: bool = True):
    """
    Load and save data to CSV for inspection.
    ...
    group : str
        M4 frequency group: 'Daily', 'Hourly', or 'Weekly'
    train : bool
        If True, save training data. If False, save test data.
    """
    df, *_ = get_data('data', group, train)
    suffix = '' if train else '-test'
    df.to_csv(f'arima_benchmark/data/m4-{group}{suffix}.csv', index=False)
    print(f"Saved {len(df)} rows to arima_benchmark/data/m4-{group}{suffix}.csv")
