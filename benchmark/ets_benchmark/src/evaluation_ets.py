"""
Exponential smoothing methods benchmarking evaluation script.

Evaluates ETS model forecasts against actual test data using MASE, MAE, and RMSE metrics.
"""
import sys
...
def evaluate(group: str = 'Daily'):
    """
    Evaluate ETS model forecasts on M4 test data.
    ...
    """
    print(f"Evaluating ETS forecasts for M4 {group} dataset...")
    ...
    statsforecast_files = list(results_dir.glob(f'statsforecast-ETS-{group}.parquet'))
    ...
    if statsforecast_files:
        print(f"\nEvaluating statsforecast ETS models...")
    ...
    # Print summary table
    print(f"\n{'='*80}")
    print(f"Exponential Smoothing Models Evaluation Summary - M4 {group}")
    print(f"{'='*80}")
