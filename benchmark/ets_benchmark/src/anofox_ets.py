"""
Anofox-forecast exponential smoothing methods benchmarking script.

Runs ETS models from the anofox-forecast DuckDB extension and saves results.
"""
import time
import sys
...
def run_benchmark(group: str = 'Daily', model: str = None):
    """
    Run anofox-forecast ETS benchmarks on M4 dataset.
    ...
    model : str, optional
        ...
        Options: 'SES', 'SESOptimized', 'SeasonalES', 'SeasonalESOptimized',
                 'Holt', 'HoltWinters', 'ETS', 'AutoETS'
    """
    models_to_run = [model] if model else ETS_MODELS
    ...
    for model_name in models_to_run:
        ...
        # Models that require seasonal_period parameter
        if model_name in ['SeasonalES', 'SeasonalESOptimized', 'HoltWinters', 'AutoETS']:
    ...
    con.close()
    print(f"\n{'='*60}")
    print("All ETS benchmarks completed")
    print(f"{'='*60}")
