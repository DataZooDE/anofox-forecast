...
from configs.ets import BENCHMARK_NAME, MODELS


def run_anofox(group: str = 'Daily'):
    """
    Run Anofox ETS benchmarks on M4 dataset.
...
def run(group: str = 'Daily'):
    """
    Run complete ETS benchmark: anofox + evaluation.
    ...
    """
    print(f"{'='*80}")
    print(f"ETS BENCHMARK - M4 {group}")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"ETS BENCHMARK COMPLETE")
    print(f"{'='*80}")
