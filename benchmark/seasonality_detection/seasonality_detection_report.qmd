...
| Multi-Method | `ts_detect_periods(values, method)` | Detection using specified method ('fft', 'acf', 'combined') |
| Multiple Periods | `ts_detect_multiple_periods(values)` | Detects all significant periods |
...
### Method 5: Multi-Method Detection (`ts_detect_periods`)

Allows selecting the detection method ('fft', 'acf', or 'combined').

```{sql}
--| label: sql-multi-method
--| connection: con
--| output.var: "multi_results"

SELECT
    global_id,
    scenario,
    true_period,
    (ts_detect_periods(values, 'combined')).primary_period as multi_period,
    (ts_detect_periods(values, 'combined')).n_periods as n_periods
FROM simulated_series
WHERE series_id = 1
ORDER BY scenario
LIMIT 7;
...
```{r}
#| label: show-multi-results

kable(multi_results, digits = 3, caption = "Multi-Method Detection Results")
```

### Method 6: Multiple Periods Detection (`ts_detect_multiple_periods`)

Detects all significant seasonal periods in the data.

```{sql}
--| label: sql-multiple-periods
--| connection: con
--| output.var: "multiple_results"

SELECT
    global_id,
    scenario,
    true_period,
    (ts_detect_multiple_periods(values)).primary_period as primary_period,
    (ts_detect_multiple_periods(values)).n_periods as n_periods
FROM simulated_series
WHERE series_id = 1
ORDER BY scenario
LIMIT 7;
...
```{r}
#| label: show-multiple-results

kable(multiple_results, digits = 3, caption = "Multiple Periods Detection Results")
```

## Running Full Detection with All Methods

Now we run all six detection methods on all simulated series.
...
WITH detection_results AS (
    SELECT
        ...
        ts_detect_periods(values, 'combined') as multi,
        -- Method 6: Multiple periods detection
        ts_detect_multiple_periods(values) as multiple
    FROM simulated_series
)
SELECT
    global_id,
    ...
    (multi).primary_period as multi_period,
    (multi).n_periods as multi_n_periods,
    -- Multiple periods results
    (multiple).primary_period as multiple_period,
    (multiple).n_periods as multiple_n_periods
...
detections <- as_tibble(all_detections) %>%
  mutate(
    ...
    multi_accurate = ifelse(true_period > 0,
                            abs(multi_period - true_period) / true_period < 0.1, FALSE),
    multiple_accurate = ifelse(true_period > 0,
                               abs(multiple_period - true_period) / true_period < 0.1, FALSE),
    ...
    multi_detected = multi_n_periods > 0,
    multiple_detected = multiple_n_periods > 0
  )
...
#| fig-cap: "Detection rates across scenarios for all six methods"
#| fig-height: 8
...
detection_rates <- detections %>%
  ...
  summarise(
    ...
    Multi = mean(multi_detected, na.rm = TRUE),
    Multiple = mean(multiple_detected, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  pivot_longer(cols = c(Basic, Analysis, FFT, ACF, Multi, Multiple),
...
detection_rates_wide <- detections %>%
  ...
  summarise(
    ...
    Multi = sprintf("%.1f%%", mean(multi_detected, na.rm = TRUE) * 100),
    Multiple = sprintf("%.1f%%", mean(multiple_detected, na.rm = TRUE) * 100),
    .groups = "drop"
  )
...
period_data <- detections %>%
  ...
  select(scenario, true_period, basic_period, analysis_period,
         fft_period, acf_period, multi_period, multiple_period) %>%
  pivot_longer(cols = ends_with("_period"),
...
metrics <- bind_rows(
  ...
  calc_metrics(detections$multi_detected, detections$is_seasonal) %>% mutate(Method = "Multi"),
  calc_metrics(detections$multiple_detected, detections$is_seasonal) %>% mutate(Method = "Multiple")
) %>%
...
method_names <- c("Basic", "Analysis", "FFT", "ACF", "Multi", "Multiple")
...
best_by_scenario <- detections %>%
  ...
  rowwise() %>%
  mutate(
    best_rate = max(c_across(Basic:Multiple)),
    best_method = names(METHOD_COLORS)[which.max(c_across(Basic:Multiple))]
  ) %>%
  ungroup() %>%
...
recommendations <- tibble(
  ...
  `Recommended Method` = c(
    ...
    "Analysis (`ts_analyze_seasonality`)",
    "Multi (`ts_detect_periods`)",
    "Multiple (`ts_detect_multiple_periods`)"
  ),
  Rationale = c(
    ...
    "Built-in detrending",
    "Cross-validates FFT and ACF",
    "Finds all significant periods"
  )
)
...
The `anofox-forecast` DuckDB extension provides robust seasonality detection through six complementary methods. Key findings:
...
- **Multi-method detection** combines approaches for reliability
- **Multiple periods detection** handles complex seasonal patterns
...
-- Method 6: Multiple period detection
