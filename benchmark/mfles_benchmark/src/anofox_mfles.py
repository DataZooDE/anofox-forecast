"""
Anofox-forecast MFLES methods benchmarking script.

Runs MFLES v2 variants from the anofox-forecast DuckDB extension and saves results.
"""
import time
import sys
...
def run_benchmark(group: str = 'Daily', model: str = None):
    """
    Run anofox-forecast MFLES benchmarks on M4 dataset.
    ...
    model : str, optional
        Specific MFLES model to run. If None, runs all models.
        Options: 'MFLES-Fast', 'MFLES-Balanced', 'MFLES-Accurate', 'MFLES-Robust', 'AutoMFLES'
    """
    models_to_run = [model] if model else MFLES_MODELS
    ...
    for model_name in models_to_run:
        ...
        # Build forecast query based on model type
        if model_name == 'AutoMFLES':
            # AutoMFLES v2 auto-selects hyperparameters using CV
            params = {'seasonal_period': seasonality}
            model_sql = 'AutoMFLES'
        else:
            # MFLES with preset configuration
            preset = model_name.replace('MFLES-', '').lower()  # 'fast', 'balanced', 'accurate', 'robust'
            params = {
                'seasonal_period': seasonality,
                'preset': f"'{preset}'"  # Need quotes around preset name
            }
            model_sql = 'MFLES'
    ...
    con.close()
    print(f"\n{'='*60}")
    print("All MFLES benchmarks completed")
    print(f"{'='*60}")
