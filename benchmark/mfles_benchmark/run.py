"""
MFLES models benchmark - configuration-driven wrapper.

Uses shared common modules and configuration files to run the MFLES benchmark.
"""
import sys
...
from configs.mfles import BENCHMARK_NAME, MODELS


def run_anofox(group: str = 'Daily'):
    """
    Run Anofox MFLES benchmarks on M4 dataset.
...
def evaluate(group: str = 'Daily'):
    """
    Evaluate MFLES model forecasts on M4 test data.
...
def run(group: str = 'Daily'):
    """
    Run complete MFLES benchmark: anofox + evaluation.
    ...
    """
    print(f"{'='*80}")
    print(f"MFLES BENCHMARK - M4 {group}")
    print(f"{'='*80}\n")
    ...
    print(f"\n{'='*80}")
    print(f"MFLES BENCHMARK COMPLETE")
    print(f"{'='*80}")
