...
CREATE OR REPLACE TABLE forecast_results AS (
    ...
    UNION ALL
    SELECT *
    FROM TS_FORECAST_BY('m5_train', item_id, ds, y, 'Theta', 28, {'confidence_level': 0.95})
    UNION ALL
    SELECT *
...
SELECT model_name, AVG(mae) AS avg_mae, AVG(bias) AS avg_bias FROM evaluation_results GROUP BY model_name;