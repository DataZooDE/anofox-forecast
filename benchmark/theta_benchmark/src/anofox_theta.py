"""
Anofox-forecast Theta methods benchmarking script.

Runs all Theta variants from the anofox-forecast DuckDB extension and saves results.
"""
import time
import sys
...
def run_benchmark(group: str = 'Daily', model: str = None):
    """
    Run anofox-forecast Theta benchmarks on M4 dataset.
    ...
    model : str, optional
        Specific Theta model to run. If None, runs all models.
        Options: 'Theta', 'OptimizedTheta', 'DynamicTheta', 'DynamicOptimizedTheta'
    """
    models_to_run = [model] if model else THETA_MODELS
    ...
    for model_name in models_to_run:
        ...
        if model_name == 'Theta':
            # Theta with default theta parameter (2.0)
            params = {'seasonal_period': seasonality, 'theta_param': 2.0}
        elif model_name == 'OptimizedTheta':
            # OptimizedTheta auto-selects theta
            params = {'seasonal_period': seasonality}
        elif model_name == 'DynamicTheta':
            # DynamicTheta with default theta parameter (2.0)
            params = {'seasonal_period': seasonality, 'theta_param': 2.0}
        elif model_name == 'DynamicOptimizedTheta':
            # DynamicOptimizedTheta auto-selects theta dynamically
            params = {'seasonal_period': seasonality}
    ...
    con.close()
    print(f"\n{'='*60}")
    print("All Theta benchmarks completed")
    print(f"{'='*60}")
