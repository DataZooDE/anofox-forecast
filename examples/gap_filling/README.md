...
|----------|---------|----------------|
| `ts_fill_gaps` | Insert missing dates with NULL | group_col, date_col, value_col |
| `ts_fill_forward` | Extend series to target date | group_col, date_col, value_col |
| `ts_drop_gappy` | Remove series with too many gaps | Original columns |
| `ts_fill_unknown` | Fill future values for CV | group_col, date_col, value_col |

---

## Patterns Overview

### Pattern 1: Fill Date Gaps (ts_fill_gaps)

**Use case:** Insert missing dates in an irregular time series.

```sql
SELECT group_col, date_col, value_col
FROM ts_fill_gaps('my_table', series_id, date, value, '1 day')
...
### Pattern 2: Fill Forward to Target Date (ts_fill_forward)

**Use case:** Extend series to a future date for forecasting.

```sql
SELECT group_col, date_col, value_col
FROM ts_fill_forward('my_table', series_id, date, value, '2024-12-31'::DATE, '1 day')
ORDER BY group_col, date_col;
```
...
### Pattern 3: Drop Gappy Series (ts_drop_gappy)
...
SELECT *
FROM ts_drop_gappy('my_table', series_id, value, 0.3);
```
...
### Pattern 4: Fill Unknown Future Values (ts_fill_unknown)
...
SELECT group_col, date_col, value_col
FROM ts_fill_unknown('features', series_id, date, feature, '2024-01-15'::TIMESTAMP,
...
SELECT * FROM ts_fill_gaps('data', id, ts, val, '1 day');
SELECT * FROM ts_fill_gaps('data', id, ts, val, '1 hour');
SELECT * FROM ts_fill_gaps('data', id, ts, val, '30 minute');

-- Polars-style format
SELECT * FROM ts_fill_gaps('data', id, ts, val, '1d');
SELECT * FROM ts_fill_gaps('data', id, ts, val, '1h');
SELECT * FROM ts_fill_gaps('data', id, ts, val, '30m');

-- Integer (days)
SELECT * FROM ts_fill_gaps('data', id, ts, val, 7);  -- 7 days
```
...
Use `ts_drop_gappy` to filter out series with high gap ratios:

```sql
-- Keep series with < 50% gaps
SELECT * FROM ts_drop_gappy('data', id, value, 0.5);
```

### Cross-Validation with Unknown Future

In time series CV, features at future dates are unknown at prediction time. Use `ts_fill_unknown` to handle this:

```sql
-- Scenario: Train on data up to 2024-01-15, predict beyond
SELECT * FROM ts_fill_unknown('features', id, date, feature,
...
4. **Cross-validation** - Use `ts_fill_unknown` for proper temporal CV.
