...
struct ForecastAggregateOperation {
	...
	static void Combine(const STATE &source, STATE &target, AggregateInputData &) {
		...
		target.data->timestamp_micros.insert(target.data->timestamp_micros.end(), source.data->timestamp_micros.begin(),
	...
	static void Finalize(STATE &state, AggregateFinalizeData &finalize_data) {
		...
		if (std::getenv("ANOFOX_PERF")) {
			...
			std::fprintf(stderr, "\n[PERF] Model=%s, Rows=%zu, Horizon=%d\n", bind_data.model_name.c_str(), data_size,
			             bind_data.horizon);
			std::fprintf(stderr, "[PERF] Sort:    %6.2fms (%5.1f%%)\n", to_ms(state.data->time_sort),
			             pct(state.data->time_sort, state.data->time_total));
			std::fprintf(stderr, "[PERF] Convert: %6.2fms (%5.1f%%)\n", to_ms(state.data->time_convert),
			             pct(state.data->time_convert, state.data->time_total));
			std::fprintf(stderr, "[PERF] BuildTS: %6.2fms (%5.1f%%)\n", to_ms(state.data->time_build_ts),
			             pct(state.data->time_build_ts, state.data->time_total));
			std::fprintf(stderr, "[PERF] Fit:     %6.2fms (%5.1f%%)\n", to_ms(state.data->time_fit),
			             pct(state.data->time_fit, state.data->time_total));
			std::fprintf(stderr, "[PERF] Predict: %6.2fms (%5.1f%%)\n", to_ms(state.data->time_predict),
			             pct(state.data->time_predict, state.data->time_total));
			std::fprintf(stderr, "[PERF] TsCalc:  %6.2fms (%5.1f%%)\n", to_ms(time_timestamp_calc),
			             pct(time_timestamp_calc, state.data->time_total));
			std::fprintf(stderr, "[PERF] Result:  %6.2fms (%5.1f%%)\n", to_ms(state.data->time_result),
			             pct(state.data->time_result, state.data->time_total));
			std::fprintf(stderr, "[PERF] TOTAL:   %6.2fms\n", to_ms(state.data->time_total));
			std::fprintf(stderr, "[PERF] Copies: %zu, Bytes: %zu KB\n", state.data->copy_count,
			             state.data->bytes_copied / 1024);
		}
	}
...
static void TSForecastUpdate(Vector inputs[], AggregateInputData &aggr_input, idx_t input_count, Vector &state_vector,
                             idx_t count) {
...
AggregateFunction CreateTSForecastAggregate() {
	...
	AggregateFunction forecast_agg(
	    "anofox_fcst_ts_forecast_agg", // name (internal, use TS_FORECAST macro for user-facing API)
