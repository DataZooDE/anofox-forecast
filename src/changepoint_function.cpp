...
struct TSDetectChangepointsOperation {
    ...
    static void Combine(const STATE &source, STATE &target, AggregateInputData &) {
        ...
        target.data->timestamps.insert(target.data->timestamps.end(), source.data->timestamps.begin(), source.data->timestamps.end());
    ...
    static void Finalize(STATE &state, AggregateFinalizeData &finalize_data) {
        ...
        for (size_t i = 0; i < state.data->values.size(); i++) {
            ...
            struct_values.push_back(make_pair("timestamp", Value::TIMESTAMP(timestamp_t(state.data->timestamps[i] * 1000))));  // Convert ms to microseconds
...
static void TSDetectChangepointsUpdate(Vector inputs[], AggregateInputData &aggr_input_data, 
    ...
    for (idx_t i = 0; i < count; i++) {
        ...
        if (!timestamp_format.validity.RowIsValid(timestamp_idx) || 
...
AggregateFunction CreateTSDetectChangepointsAgg() {
    ...
    // Return type: LIST<STRUCT(timestamp TIMESTAMP, value DOUBLE, is_changepoint BOOLEAN, changepoint_probability DOUBLE)>
