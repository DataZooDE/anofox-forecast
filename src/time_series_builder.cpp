...
std::unique_ptr<::anofoxtime::core::TimeSeries> TimeSeriesBuilder::BuildTimeSeries(
    const std::vector<std::chrono::system_clock::time_point>& timestamps,
    const std::vector<double>& values
) {
    // std::cerr << "[DEBUG] TimeSeriesBuilder::BuildTimeSeries (vector version) with " << timestamps.size() << " points" << std::endl;
...
void TimeSeriesBuilder::ValidateTimeSeriesData(
    const DataChunk& timestamp_chunk,
    const DataChunk& value_chunk,
    idx_t start_idx,
    idx_t count
...
void TimeSeriesBuilder::ValidateValues(const DataChunk& value_chunk, idx_t start_idx, idx_t count) {
    ...
    for (idx_t i = start_idx; i < start_idx + count; i++) {
        ...
        if (!IsFinite(val)) {
            throw InvalidInputException("Value at index " + std::to_string(i) + " is not finite: " + std::to_string(val));
        }
    }
}
