...
static void TSDetectSeasonalityFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t i = 0; i < args.size(); i++) {
		...
		try {
			auto detector = seasonality::SeasonalityDetector::builder().minPeriod(4).threshold(0.9).build();
...
static void TSAnalyzeSeasonalityFunction(DataChunk &args, ExpressionState &state, Vector &result) {
	...
	for (idx_t i = 0; i < args.size(); i++) {
		...
		if (!timestamps_format.validity.RowIsValid(timestamps_idx) || !values_format.validity.RowIsValid(values_idx)) {
			result_validity.SetInvalid(i);
			continue;
		}
